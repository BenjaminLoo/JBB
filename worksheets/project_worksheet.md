...
-   Q1. Consider the use case of `Compute basic/advance result`, **HOW** is this functionality made available to both JiBaBoom and Adminstrator _(Hint: They are different)_? `JiBaBoom uses the API at the backend to compute the result and the admin can call the result from the frontend and mobile.` 
-   Q2. Consider the use case of `Insert basic/advance data`. **HOW** is the data stored? `The data is stored into the database through the backend`
...
3. How should the frontend display data from the database? `In a data table with pagination and filtering`
4. How does the frontend communicate with backend? `It sends a request to the backend api and receives the response from the backend server`
...
```
[Create an API that retrieves a certain amount of rows from the database depending on the page size and number and other options.]
```
